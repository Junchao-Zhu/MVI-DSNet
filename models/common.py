...
from .networks.TCSeg import TCSeg
...
class MobileNetV3s1(nn.Module):
    ...
    def __init__(self, ignore) -> None:
        ...
        model.load_state_dict(torch.load('/export/home/bs2021/zhujunchao/ours/MVISeg/weight/previous_version/ablation_exp/mobilenet_v3_small.pth'))
...
class resnet341(nn.Module):
    def __init__(self, ignore) -> None:
        ...
        model.load_state_dict(torch.load('/export/home/bs2021/zhujunchao/ours/MVISeg/weight/previous_version/ablation_exp/resnet34.pth'))
        modules = list(model.children())
        modules = modules[:6]
        self.model = nn.Sequential(*modules)
...
class resnet501(nn.Module):
    def __init__(self, ignore) -> None:
        ...
        model.fc = nn.Linear(model.fc.in_features, 3)
        model.load_state_dict(torch.load('/export/home/bs2021/zhujunchao/ours/MVISeg/weight/previous_version/ablation_exp/resnet50.pth'))
        modules = list(model.children())
        modules = modules[:6]
        self.model = nn.Sequential(*modules)
...
class tcseg1(nn.Module):
    def __init__(self, ignore) -> None:
        super().__init__()
        self.model = TCSeg()
        self.resblock = ResBlock(in_channels=112, out_channels=192)

        self.model.load_state_dict(torch.load('/export/home/bs2021/zhujunchao/ours/MVISeg/weight/previous_version/baseline_batch_size_4_lr0.0075both_have/epoch315.pth'))
        # self.model.load_state_dict(torch.load('/export/home/bs2021/zhujunchao/ours/MVISeg/weight/baseline_batch_size_4_lr0.0075no_cls_loss/epoch125.pth'))
        # self.model.load_state_dict(torch.load('/export/home/bs2021/zhujunchao/ours/MVISeg/weight/baseline_batch_size_2_lr0.0075no_mask_loss/epoch104.pth'))

    def forward(self, x):
        x, x_c, _, _ = self.model(x)
        x = torch.cat([self.resblock(x), x_c], dim=1)
        return x


class tcseg2(nn.Module):
    def __init__(self, ignore) -> None:
        super().__init__()
        self.resblock = ResBlock(in_channels=256, out_channels=512)
...
class tcseg3(nn.Module):
    def __init__(self, ignore) -> None:
        super().__init__()
        self.resblock = ResBlock(in_channels=512, out_channels=1024)
